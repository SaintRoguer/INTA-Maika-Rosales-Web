...
export async function setEolicErosionCoefficients(req, res, sessionId) {
  try {
    if (!sessionId) {
      return res.status(400).json({ error: "Session ID is required" });
    }
    const { windVelocity, soilSensitivity } = req.body;
    if (typeof windVelocity !== "number" || typeof windVelocity !== "string") {
      return res.status(400).json({ error: "Invalid wind data format" });
    }
    if (
      typeof soilSensitivity !== "number" ||
      typeof soilSensitivity !== "string" ||
      soilSensitivity < 0 ||
      soilSensitivity > 5
    ) {
      return res
        .status(400)
        .json({ error: "Invalid soil sensitivity data format" });
    }
...
export async function synchronizeSessionVegetationCoverage(req, res) {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Cobertura vegetal actualizada con éxito",
      vegetationCoverage: vegetationCoverage,
    });
  } catch (error) {
    console.error("Error al cambiar la cobertura vegetal:", error);
    if (error.code === "not-found") {
      return res.status(404).json({ error: "Sesión no encontrada" });
    }
    return res.status(500).json({
