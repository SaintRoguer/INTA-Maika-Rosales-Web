...
export async function updateUserData({
  ...
  try {
    ...
    if (photoUrl !== undefined) userUpdates.photoUrl = photoUrl; //Undefined es para controlar los strings vacios y no poner ninguna foto
...
export async function getAllRoles() {
  const sessionsSnapshot = await db.collection("roles").get();
...
export async function getSharedSessions(req, res) {
  ...
  try {
    ...
    for (let i = 0; i < sharedSessionIds.length; i += chunkSize) {
      ...
      queryPromises.push(
        sessionsRef.where("id", "in", chunk).orderBy("date", "desc").get()
      );
    }
  ...
  try {
    ...
    if (!sessionId || !Array.isArray(sharedUsers)) {
      return res.status(400).json({
        error:
          "Missing required parameters: sessionId and sharedUsers must be provided",
      });
    }
...
const host = process.env.NEXT_PUBLIC_EMAIL_HOST || "smtp.gmail.com";
const port = parseInt(process.env.NEXT_PUBLIC_EMAIL_PORT) || 465;
...
transporter
  .verify()
  .then(() => console.log(`Connected to SMTP at ${host}:${port}`))
  .catch((err) => {
...
export async function resetPassword(req) {
  ...
  if (!baseUrl || typeof baseUrl !== "string") {
    throw new Error("NEXT_PUBLIC_SITE_URL is not properly configured");
  }

  const continueUrl = new URL("/sign-in", baseUrl).toString();
...
async function sendPasswordResetEmail(email, resetLink) {
  const mailOptions = {
    from: `"INTA Web" <${process.env.NEXT_PUBLIC_SITE_URL}>`,
    to: email,
    subject: "Pedido de restablecimiento de contraseña",
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #333;">Password Reset</h2>
        <p>Hemos recibido un pedido de restrablecimiento de contraseña. Clickee el boton que se encuentra debajo para proceder:</p>
        <a href="${resetLink}" 
           style="display: inline-block; padding: 10px 20px; background-color: #4CAF50; 
                  color: white; text-decoration: none; border-radius: 5px; margin: 20px 0;">
          Restablecer contraseña
        </a>
        <p>Si no has pedido esto, ignorar este email.</p>
        <p style="font-size: 12px; color: #777;">El pedido expirara en 1 hora.</p>
      </div>
    `,
  };
