...
export async function updateSession(
  ...
  try {
    ...
    console.error("Error al actualizar la descripción sesión:", error);
    throw error;
  }
}

export async function updateLote(loteDetailId, newDescription) {
  try {
    const lotesDetails = "lotesDetails";
    const loteDetailRef = doc(db, lotesDetails, loteDetailId);

    await updateDoc(loteDetailRef, { description: newDescription });
  } catch (error) {
    console.error("Error al actualizar la descripción del lote:", error);
    throw error;
  }
}

export async function updatePastura(pasturaDetailId, newDescription) {
  try {
    debugger;
    const pasturasDetails = "pasturasDetails";
    const pasturaDetailRef = doc(db, pasturasDetails, pasturaDetailId);

    await updateDoc(pasturaDetailRef, { description: newDescription });
  } catch (error) {
    console.error("Error al actualizar la descripción de la pastura:", error);
    throw error;
  }
}
...
export async function getRoleFromToken(req) {
